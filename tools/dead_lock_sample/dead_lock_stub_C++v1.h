...
#define WAIT_MUTEXLOCK_TIMEOUT 10
...
public:
    ...
    void thread_hold_lock_show(uint64_t local_addr) const{
        hold_lock_point_t *p;
        printf("%slock_name(lock_addr)%s: %s%s%s(%llu), count: %s%d%s\n", COLOR_GREEN, COLOR_END, COLOR_RED, this->lock_name, COLOR_END, local_addr, COLOR_GREEN, this->count, COLOR_END);
