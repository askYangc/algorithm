...
struct SignalImpl : boost::noncopyable
{
  ...
  void copyOnWrite()
  {
    ...
    if (!slots_.unique())
    {
      slots_.reset(new SlotList(*slots_));	//copy on write，有其他线程在使用slots，复制的是vector，里面是weak_ptr
    }
  ...
  MutexLock mutex_;
  boost::shared_ptr<SlotList> slots_;		//一个vector的shared_ptr，vector保存的是weak_ptr(指向SlotImpl)
};
...
class Signal<RET(ARGS...)> : boost::noncopyable			/* singal<RET(ARG...)> 这种叫类模板的偏特化 */
{
 public:
 ...
 private:
  ...
  const boost::shared_ptr<SignalImpl> impl_;	//一个SignalImpl的智能指针，impl_里面有一个vector
};
