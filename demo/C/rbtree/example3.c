...
struct memfs_file *memfs_file_init(char *path, char *data)
{
		
	struct memfs_file *f =  (struct memfs_file*)calloc(1, sizeof(struct memfs_file));
	if(f) {
		f->path = path;
		f->data = data;
	}
...
int main(int argc, char *argv[])
{
	...
	rb_search_or_insert(&root, f, struct memfs_file, node, memfs_file_search, "e,png", memfs_file_init, "e.png", "0x4321");
