...
#define stlc_list_last_entry(ptr, type, member) \
	stlc_list_entry((ptr)->prev, type, member)
...
#define stlc_list_for_each_prev_entry(type, pos, head, member) \
	for (pos = stlc_list_entry((head)->prev, type, member),	\
		     prefetch_load(pos->member.prev);			\
	     &pos->member != (head); 					\
	     pos = stlc_list_entry(pos->member.prev, type, member),	\
		     prefetch_load(pos->member.prev))
 ...
 * @member:	the name of the list_struct within the struct.
 */
#define stlc_list_for_each_entry(type, pos, head, member)				\
	for (pos = stlc_list_entry((head)->next, type, member),	\
		     prefetch_load(pos->member.next);			\
	     &pos->member != (head); 					\
	     pos = stlc_list_entry(pos->member.next, type, member),	\
		     prefetch_load(pos->member.next))
 ...
 * @member:	the name of the list_struct within the struct.
 */
#define stlc_list_for_each_entry_safe(type, pos, n, head, member)			\
	for (pos = stlc_list_entry((head)->next, type, member),	\
		n = stlc_list_entry(pos->member.next, type, member);	\
	     &pos->member != (head); 					\
	     pos = n, n = stlc_list_entry(n->member.next, type, member))

/**
 * stlc_list_free - 释放所有节点，链表头不动
 * @type:   节点类型
 * @head:	the head for your list.
 * @member:	the name of the list_struct within the struct.
 */
#define stlc_list_free(type, head, member)			\
	do { \
		type *pos, *n; \
		for (pos = stlc_list_entry((head)->next, type, member),	\
			n = stlc_list_entry(pos->member.next, type, member);	\
...
/*
	根据数组下标取成员
*/
#define	stlc_list_by_index(type, pos, head, member, index) \
	do { \
		int count = 0; \
		for (pos = stlc_list_entry((head)->next, type, member);	\
		     &pos->member != (head) && count < (int)(index); 					\
		     pos = stlc_list_entry(pos->member.next, type, member), count++) \
		{ } \
		if (&pos->member == (head) || count != index) \
			pos = NULL; \
	} while (0)

#define	stlc_list_count(count, head) \
	do { \
		struct stlc_list_head *node; \
		for (node = (head)->next, count = 0; node != (head); node = node->next, count++) \
			; \
	} while (0)
